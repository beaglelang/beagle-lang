...
impl<'a> Parser<'a>{
    ...
    pub(crate) fn emit_ir(&mut self, pos: Position, sig: TypeSignature, ins: Instruction){
        ...
        self.ir_tx.send(Some(ir)).expect(format!("Failed to send IR through IR channel.").as_str())
    }
    ...
    #[inline]
    pub(crate) fn consume(&mut self, type_: TokenType, error_message: &'static str) -> Result<&TokenData, ()>{
        if self.check(type_){
            self.advance().unwrap();
